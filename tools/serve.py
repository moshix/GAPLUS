...
def main():
    ...
    with ThreadingHTTPServer(("127.0.0.1", port), handler) as httpd:
        # Keep every line within 79 columns (the owner's console rule).
        print(f"Gaplus: http://localhost:{port}/  (caching disabled)")
        print(f"serving {root}"[:79])
        print("5 = coin, 1 = start, arrows move, space fires.  Ctrl-C to stop.")
        try:
            httpd.serve_forever()
